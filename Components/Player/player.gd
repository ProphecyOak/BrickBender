...
func checkForInputs():
	...
	crouch(MultiplayerInput.get_action_raw_strength(deviceNum, "Crouch") > .4 or 
		MultiplayerInput.is_action_just_pressed(deviceNum, "Crouch"))
	if (MultiplayerInput.get_action_raw_strength(deviceNum, "Jump") > .7 or 
		MultiplayerInput.is_action_just_pressed(deviceNum, "Jump")): jump()
